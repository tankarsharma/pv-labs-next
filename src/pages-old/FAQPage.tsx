...
const FAQPage = () => {
  ...
  const filtered = (active === "All" ? faqs : faqs.filter((f) => f.cat === active)).filter(
    (f) =>
        ...
        <div className="max-w-7xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto mb-8">
              Buyer questions about pricing, conversion, compliance, storefront support, and choosing the right service path.
            </p>
            <div className="max-w-md mx-auto relative">
              ...
              <input
                ...
                className="w-full bg-background border border-border rounded-full pl-12 pr-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
              />
            </div>
          </motion.div>
        </div>
          ...
          {filtered.map((f, i) => (
            ...
            >
              <summary className="font-heading font-bold text-foreground cursor-pointer list-none flex items-center justify-between gap-4">
                ...
                <span className="text-primary ml-2 group-open:rotate-45 transition-transform text-xl flex-shrink-0">
                  +
                </span>
              </summary>

              <div className="mt-4 pl-9">
                <span className="text-xs text-primary bg-primary/10 px-2 py-0.5 rounded-full mb-2 inline-block">
                  {f.cat}
                </span>
        ...
        <div className="section-padding bg-gradient-to-r from-slate-900 to-slate-800 text-center m-5 max-w-4xl mx-auto rounded-md">
          <div className="max-w-3xl mx-auto ">
            ...
            <p className="text-primary-foreground/80 mb-8">
              If you are comparing pricing, services, or next steps, request a creative audit and we will guide you.
            </p>
            <Link
              href="/contact"
              className="bg-background text-foreground px-8 py-4 rounded-full font-semibold inline-flex items-center gap-2 hover:shadow-xl transition-all"
            >
